...
anahtarlar = [machine.Pin(i, machine.Pin.IN, machine.Pin.PULL_DOWN) for i in range(7, 14)]
durumlar = [False] * 7
...
def pin_change_handler(pin):
    # Pin numarasını al
    pin_number = list(pin_mapping.keys())[list(pin_mapping.values()).index(pin)]
    
    # İlgili çıkış pini numarasını al
    output_pin_number = (pin_number - 7) % 7
    
    # Çıkış pini durumunu değiştir
    pin_states[output_pin_number] = 1 - pin_states[output_pin_number]
    
    # İlgili çıkış pini durumunu uygula
    pin_mapping[output_pin_number].value(pin_states[output_pin_number])
...
def check_pin_states():
    for input_pin_number in range(7, 14):
        pin = pin_mapping[input_pin_number]
        current_state = pin_states[input_pin_number - 7]
        new_state = pin.value()

        if new_state != current_state:
            output_pin_number = (input_pin_number - 7) % 7
            pin_states[output_pin_number] = new_state
            pin_mapping[output_pin_number].value(new_state)
